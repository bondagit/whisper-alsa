...
bool Capture::open(const std::string &device, uint32_t rate, uint8_t channels) {
  if (is_open_) {
    BOOST_LOG_TRIVIAL(error) << "capture:: audio device already open";
    return false;
  }
  int err;
  if ((err = snd_pcm_open(&capture_handle_, device.c_str(),
                          SND_PCM_STREAM_CAPTURE, SND_PCM_NONBLOCK)) < 0) {
    BOOST_LOG_TRIVIAL(fatal) << "capture:: cannot open audio device " << device
                             << " : " << snd_strerror(err);
    return false;
  }
